...
def assigned(tenant, staff):
    return len(StaffTenantLink.query.filter(
...
class TestStaffTenant:
    def test_when_provided_no_staff_params(self, valid_header, create_tenant, create_join_staff):
        tenant_1 = create_tenant()
        tenant_2 = create_tenant()
        keep_tenant = create_tenant()
        StaffTenantLink(tenant_id=tenant_1.id, staff_id=create_join_staff().id).save_to_db()
        StaffTenantLink(tenant_id=tenant_1.id, staff_id=create_join_staff().id).save_to_db()
        StaffTenantLink(tenant_id=tenant_2.id, staff_id=create_join_staff().id).save_to_db()
        StaffTenantLink(tenant_id=keep_tenant.id, staff_id=create_join_staff().id).save_to_db()
        ...
        assert response.json == {'message': 'Staff-Tenant associations updated successfully'}
        assert len(StaffTenantLink.query.all()) == 1

    def test_when_provided_staff_params(self, valid_header, create_tenant, create_join_staff):
        tenant = create_tenant()
...
class TestStaffTenantAuthorizations:
    ...
    def test_staff_is_unauthorized(self, staff_header, create_tenant, create_join_staff):
        response = self.client.patch(
            endpoint(),
            json=valid_payload(
                tenants=[create_tenant().id],
