...
def seedData():
    ...
    db.session.add(newTicket)
    newTicket = TicketModel(issue="Running a SpeakEasy", tenant=2, sender=4, status="Critical", urgency="HIGH", assignedUser=4)
    db.session.add(newTicket)
    newTicket = TicketModel(issue="Smoking", tenant=2, sender=3, status="Critical", urgency="HIGH", assignedUser=6)
    db.session.add(newTicket)
    newTicket = TicketModel(issue="After Hours Noise", tenant=2, sender=3, status="Critical", urgency="HIGH", assignedUser=4)
    db.session.add(newTicket)
    newTicket = TicketModel(issue="Unregistered Resident", tenant=2, sender=4, status="Critical", urgency="HIGH", assignedUser=4)
    db.session.add(newTicket)
    newTicket = TicketModel(issue="Tenant Disputes", tenant=2, sender=4, status="Critical", urgency="HIGH", assignedUser=5)
    db.session.add(newTicket)
    newTicket = TicketModel(issue="Pet Possum?", tenant=2, sender=3, status="Critical", urgency="HIGH", assignedUser=5)
    db.session.add(newTicket)
