...
class Seed:
    ...
    def data(cls):
        ...
        for _ in range(7):
            Admin.create(
                schema=UserSchema,
                payload=cls.user_attributes(faker, role=RoleEnum.ADMIN.value),
            )
        ...
        for _ in range(2):
            Admin.create(
                schema=UserSchema,
                payload=cls.user_attributes(
                    faker, role=RoleEnum.ADMIN.value, archived=True
                ),
            )
        ...
        for _ in range(5):
            UserModel.create(
                schema=UserRegisterSchema, payload=cls.user_attributes(faker)
            )
        ...
        for _ in range(3):
            UserModel.create(
                ...
                payload=cls.user_attributes(faker, archived=True),
            )
